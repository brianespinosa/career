...
- `@types/d3-shape` is declared as an explicit `devDependency`, pinned to the same major version as `d3-shape`. It was previously resolved only by chance: two conflicting versions exist transitively (`1.3.12` via `@visx/sankey`, `3.1.7` via `@visx/vendor`), and pnpm's isolated linker can only hoist one to the root `node_modules/@types/d3-shape` TypeScript reads from — which one wins was not guaranteed by the lockfile and differed between local (macOS) and CI (Linux) runs during the pnpm migration (`docs/adr/013-pnpm-migration.md`)
